...
Manage Jira tasks, epics, and ticket status for project `PET` from Claude Code.
...
| `/jira status PET-123` | Summarise a specific ticket |
| `/jira validate` | Validate the current-branch ticket against standards |
| `/jira validate PET-123` | Validate a specific ticket against standards |
| `/jira search [query]` | Search issues by label, status, assignee, etc. |
| `/jira worklog PET-123 2h` | Log hours on a ticket |
| `/jira timesheet` | Show today's logged hours across all projects |
| `/jira sprint PET-123` | Assign a ticket to the active sprint |
| `/jira transition PET-123` | Move a ticket to a new status |
| `/jira help` | Show this help |

**Auto-triggers (no command needed):**
- Mentioning a `PET-xxx` key, or asking "what's left" → status
...
**Prerequisites:**
- `.claude/jira-config.md` - project key (`PET`), story-points field, branch format (auto-created on first use)
