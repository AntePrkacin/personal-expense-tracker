...
- **Never hand-edit a generated-but-committed artifact.** That is `backend/openapi.json`,
  ...
  PET-80 added four more, all under `docs/showcase/`: `charts.js`, `data/all.data.js` and
  `data/*.json`, regenerated by `mise run showcase:stats`, plus `data/tests.json` from
  `mise run showcase:stats:tests`. They are committed so the statistics page renders from a
