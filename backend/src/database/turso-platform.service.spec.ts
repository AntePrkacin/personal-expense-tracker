...
describe('TursoPlatformService', () => {
  ...
  describe('createUserDatabase', () => {
    it('posts to the organization scoped endpoint with the configured group', async () => {
      respond({
        database: {
          Name: 'expenso-user-1',
          Hostname: 'expenso-user-1-acme.aws.turso.io',
        },
      });

      const result = await service.createUserDatabase('expenso-user-1');
    ...
    it('always requests the Turso engine, never the libSQL default', async () => {
      ...
      await service.createUserDatabase('expenso-user-1');
    ...
    it('fails loudly when Turso returns no hostname', async () => {
      respond({ database: { Name: 'expenso-user-1' } });

      await expect(
        service.createUserDatabase('expenso-user-1'),
  ...
  describe('mintDbToken', () => {
    it('requests a full-access token that never expires', async () => {
      ...
      await expect(service.mintDbToken('expenso-user-1')).resolves.toBe(
        'db-token',
      );

      const [url, init] = lastCall();
      expect(url).toBe(
        'https://api.turso.tech/v1/organizations/acme/databases/expenso-user-1/auth/tokens?authorization=full-access&expiration=never',
      );
      expect(init.method).toBe('POST');
    });
    ...
    it('fails loudly when Turso returns no token', async () => {
      ...
      await expect(service.mintDbToken('expenso-user-1')).rejects.toThrow(
  ...
  describe('deleteUserDatabase', () => {
    it('deletes by name', async () => {
      ...
      await service.deleteUserDatabase('expenso-user-1');

      const [url, init] = lastCall();
      expect(url).toBe(
        'https://api.turso.tech/v1/organizations/acme/databases/expenso-user-1',
      );
  ...
  it('bounds every request with a timeout, so a hung call cannot stall registration', async () => {
    ...
    await service.mintDbToken('expenso-user-1');
  ...
  it('turns an API error into a generic 500 rather than leaking the response', async () => {
    ...
    await expect(service.createUserDatabase('expenso-user-1')).rejects.toThrow(
      InternalServerErrorException,
    );
  });
});
