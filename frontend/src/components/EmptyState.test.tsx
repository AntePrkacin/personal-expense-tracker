...
describe('EmptyState', () => {
  ...
  it('hides the icon and its circle from the accessible tree, whatever the caller passes', () => {
    // The heading carries the meaning. Same call CategoryChip's dot and ui/Input's $ prefix
    // make - except those own their own glyph, and this component takes one from outside. So
    // the guarantee has to be enforced here rather than requested in a doc comment: `Glyph`
    // above asks to be announced, and this asserts it still is not.
    const { container } = render(
