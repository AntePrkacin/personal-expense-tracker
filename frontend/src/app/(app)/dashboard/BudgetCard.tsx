    ...
    <section className="card bg-base-100 shadow-sm">
      <div className="card-body gap-4">
        ...
        <div className="flex items-baseline gap-2">
          <p className="font-display text-4xl font-bold">{formatWhole(spent)}</p>
          <p className="text-base-content/60 text-sm">of {formatWhole(monthlyBudget)}</p>
        </div>
        ...
        <div className="flex items-center justify-between text-sm">
          {/* The magnitude, not a formatted negative: overspent, `remaining` is negative, and
              ...
              says "Over budget". */}
          <p>{formatWhole(Math.abs(remaining))} left</p>
          <p className="text-base-content/60">
            {daysLeft} days left in {monthLabel(new Date())}
          </p>
        </div>
