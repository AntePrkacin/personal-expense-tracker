 ...
 *
 * **`missing` promises a refresh and this modal delivers one**, which is the defect PET-38's review
 * found in the same arm next door: the copy said "close this to see the current list" and nothing
 * re-read, so the stale card stayed in the grid forever. It also says nothing was saved, which is a
 * promise the endpoint actually makes - the whole payload is refused when any id is dead, so the
 * user's other edits are intact and retrying after a refresh is safe.
 */
const MESSAGES = {
...
export function AllocateBudgetModal({
  ...
  const assigned = assignedCents(draft, ledger);
  const unassigned = unassignedCents(draft, ledger);
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      result = await save(toAllocateBody(original, draft));
    } catch {
      setPending(false);
    ...
    if (!result.ok) {
      ...
      // The one arm that refreshes, `EditCategoryModal`'s precedent: `missing` means the grid behind
      // this dialog is drawing a category the server no longer has, which is exactly what the copy
      // says closing will fix. The other three changed nothing on the server. The modal stays open
      // either way, because the user has a screen of edits in front of them and this line is what
      // explains why they could not be saved.
      ...
      footer={
        <>
          ...
          {/* Disabled until something really changed, so the primary action cannot fire a request
              ...
              dead button with no explanation beside it is worse than a rejected submit. */}
          <Button type="submit" label="Save caps" disabled={pending || !isDirty(original, draft)} />
        </>
      }
    >
      ...
      <section className={ISLAND}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex flex-col gap-1">
            ...
            {/* Never negative and never in a danger tone - `unassignedCents` clamps at zero, which
                is the whole of the design's rule. */}
            <span className="font-display text-3xl font-bold">{formatWhole(unassigned / 100)}</span>
          </div>

          <dl className="flex min-w-56 flex-col gap-1.5 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-base-content/60">Monthly budget</dt>
              <dd className="font-medium">{formatWhole(ledger.budgetCents / 100)}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-base-content/60">Assigned to categories</dt>
              <dd className="font-medium">{formatWhole(assigned / 100)}</dd>
            </div>
            <div className="border-base-300 flex justify-between gap-4 border-t pt-1.5">
              <dt className="font-medium">Unassigned</dt>
              <dd className="font-semibold">{formatWhole(unassigned / 100)}</dd>
            </div>
          </dl>
        </div>
                ...
                <div className="flex min-w-0 flex-col">
                  ...
                  >
                    {over === null
                      ...
                      : `${formatWhole(row.spent)} spent · ${formatWhole(over / 100)} over this cap`}
                  </span>
                </div>
      ...
      {snap === null ? (
        ...
        <p role="status" className="text-warning text-xs">
          {cappedMessage(snap.cents, ledger.budgetCents)}
        </p>
      )}
