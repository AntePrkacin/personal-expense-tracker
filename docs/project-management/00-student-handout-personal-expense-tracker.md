...
You have three inputs: the **app brief** (what the app is), the **tech spec** (what to build, screen by screen), and the **Figma design** (what it looks like). You'll turn them into a Jira board: **7 epics** and roughly **28 to 38 tasks**. This handout is the whole method on a few pages, with one finished example to copy.
...
| Assumption | A numbered working decision (A1 to A44, spec section 6) where the design left a gap |
...
5. **Track coverage** with the checklist at the end of this handout. Cross off each requirement ID once a task covers it. You're done when all 101 are crossed off.
...
|---|---|---|---|
| Onboarding | 2.1 to 2.6 | 01, 02, 03, 22, 23, 24 | `onboarding` |
| App shell and navigation | DSH-1 and DSH-2 (in 2.7) | sidebar and header, on every screen | `shell` |
| Dashboard | 2.7 (the rest) | 04, 05 | `dashboard` |
| Transaction logging and management | 2.8 to 2.13 | 06 to 12 | `transactions` |
| Category budgets | 2.14, 2.15 | 13, 18, 19, 20, 21 | `categories` |
| AI Insights | 2.16 | 14, 15, 16 | `insights` |
| Settings | 2.17 | 17 | `settings` |

Onboarding is the epic that grew: it now owns account registration as step 3 of 3 (22) and the whole passwordless access flow (23, 24), not just the two setup steps. Category budgets owns the category editor (18 to 21) as well as the cards.
...
| Labels | `frontend` on every task. Add `design-review` when your context cites an assumption (A1 to A44), it means a designer still owes an answer |
...
|---|---|---|
| Highest | Nothing works without it | Saving an expense (ADD-2, ADD-3): dashboard, categories, and insights all feed off transactions. Registration and link sign-in (REG-4, VER-4) sit in front of every one of those screens |
...
Two more reference points from the screens added later: the category add, edit, and delete set (CED-1 to CED-9) is a 5, and passwordless access end to end (REG, LOG, VER) is an 8, because it spans three screens plus emailed links and sessions.
...
- **Description:** Everything that lets a user create, browse, inspect, correct, and remove expenses. Covers spec sections 2.8 to 2.13. Success: a user can go from zero expenses to a maintained log without leaving the Transactions area.
...
3. All 101 requirement IDs on the checklist are crossed off.
4. No task is bigger than 8 points, and you used at least three priority levels.
5. Every task that cites an assumption (A1 to A44) carries the `design-review` label.

## Four traps

1. **Inventing features.** No password field, no calendar date picker, no month navigation: the design has none of these. Access is by emailed link only, and the date field is a closed select. If it has no screen and no requirement ID, it's not a task. Park it for the designer.
...
Print this, cross off each ID when a task covers it. 101 in total.
...
| Register user (2.4) | REG-1, REG-2, REG-3, REG-4, REG-5, REG-6 |
| Log in (2.5) | LOG-1, LOG-2, LOG-3, LOG-4, LOG-5, LOG-6 |
| Check your email (2.6) | VER-1, VER-2, VER-3, VER-4, VER-5 |
| Dashboard (2.7) | DSH-1, DSH-2, DSH-3, DSH-4, DSH-5, DSH-6, DSH-7, DSH-8, DSH-9 |
| Transactions - List and empty (2.8) | TRN-1, TRN-2, TRN-3, TRN-4, TRN-5, TRN-6, TRN-7, TRN-8, TRN-9 |
| Transaction detail (2.9) | DET-1, DET-2, DET-3, DET-4, DET-5, DET-6, DET-7, DET-8 |
| Add transaction (2.10) | ADD-1, ADD-2, ADD-3, ADD-4, ADD-5, ADD-6, ADD-7 |
| Edit transaction (2.11) | EDT-1, EDT-2, EDT-3, EDT-4 |
| Row menu (2.12) | MNU-1, MNU-2 |
| Delete confirmation (2.13) | DEL-1, DEL-2, DEL-3 |
| Categories (2.14) | CTG-1, CTG-2, CTG-3, CTG-4, CTG-5, CTG-6 |
| Category management (2.15) | CED-1, CED-2, CED-3, CED-4, CED-5, CED-6, CED-7, CED-8, CED-9 |
| AI Insights (2.16) | INS-1, INS-2, INS-3, INS-4, INS-5, INS-6, INS-7 |
| Settings (2.17) | SET-1, SET-2, SET-3, SET-4, SET-5, SET-6 |
