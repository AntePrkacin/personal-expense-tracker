...
| `repo-gcp`        | Driving Google Cloud through `gcloud` in Bash: when it loads, the coordinates, and the traps that bit this project. The runbook itself lives in `docs/guides/deployment.md`. Replaced `repo-fly` at PET-86, when the backend moved to Cloud Run |
